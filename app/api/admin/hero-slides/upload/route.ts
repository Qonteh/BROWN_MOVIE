...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.type.startsWith('image/')) {
      return NextResponse.json({ error: 'Only image files are allowed for hero slides' }, { status: 400 })
    }
    ...
    console.error('Hero slide upload error (database mode):', error)

    return NextResponse.json(
      {
        error: 'Failed to process hero image upload',
      },
      { status: 500 },
    )
  }
